{ Windows Auto Key modified for LLCL
0.111 Fixed WinActivate and WinSetState
0.110 removed variants
0.107
}
...
const
  ...
  VK_SHIFT_DOWN   = VK_RETURN;  //Use either VK_SHIFT or VK_SHIFT_DOWN
  VK_SHIFT_DN     = VK_SHIFT;
  VK_SHIFT_UP     = VK_SHIFT or KF_UP;

  VK_CONTROL_DOWN = VK_CONTROL;
  VK_CONTROL_DN   = VK_RETURN;
  VK_CONTROL_UP   = VK_CONTROL or VK_RETURN;
...
Procedure Send(const Keys: Array of Word; const RepeatCount: Integer=1);
Procedure Send(const Keys: Array of String; const RepeatCount: Integer=1);
...
begin
  Input := Default(TInput);
  Input.type_ := INPUT_MOUSE;
  Input.mi.dx := 0 {MulDiv(X, 65535, Screen.Width - 1)};  //horizontal
  Input.mi.dy := 0 {MulDiv(Y, 65535, Screen.Height - 1)}; //vertical
...
begin
  ...
  if IsWindowVisible(h) then begin
    //EnumChildWindows(h, WNDENUMPROC(@hCtlCallBack), LPARAM(@Ctl)); //{%H-}LPARAM(@Ctl)
    EnumChildWindows(h, @hCtlCallBack, LPARAM(@Ctl)); //{%H-}LPARAM(@Ctl)
  end;
...
begin
  ...
  //Debugln('FindTitle2='+Win^.FindTitle);
  //Debugln('h='+IntToHex(h,4));
  ////b:=IsWindowVisible(h);
  if b then Debugln('b=TRUE');

  if IsWindowVisible(h) then begin

    //if Win^.FindTitle<>'' then Debugln('ok 1');

    if Win^.FindTitle<>'' then begin

      //Debugln('FindTitle3='+Win^.FindTitle);
      //Application.ProcessMessages;

      GetWindowTextW(h, LPWSTR(WBuf), 32 {MAX_PATH});

      //WinTitle:=TrimRight(UnicodeString(WBuf));
      WinTitle:=WBuf;

      //Debugln('WinTitle=['+WinTitle+']');

      MatchTitle:=_DoMatch(WAK.TitleMatchMode, Win^.FindTitle, WinTitle);

      //if MatchTitle then Debugln('Title='+WinTitle);
...
begin
  ...
  //EnumWindows(@hWinCallBack, LPARAM(@Win));
...
initialization
  ...
  WAK.TitleMatchMode:=1;     //mtStartsWith=1
  WAK.TextMatchMode:=2;      //mtSubString=2
